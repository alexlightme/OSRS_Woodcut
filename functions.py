...
def get_runelite_dimensions(Name):
    runelite = pyautogui.getWindowsWithTitle(f"RuneLite - {Name}")[0]
    bottomleft = runelite.bottomleft
    bottomright = runelite.bottomright
    topleft = runelite.topleft
    topright    = runelite.topright
    dimensions =[bottomleft,bottomright, topleft, topright]
    return dimensions

def get_runelite(Name):
    runelite = pyautogui.getWindowsWithTitle(f"RuneLite - {Name}")[0]
    return runelite
...
def image_Rec_clicker(image, event, iheight=5, iwidth=5, threshold=0.7, clicker='left', ispace=20, playarea=False,
                      fast=False, left=0, top=0, right=0, bottom=0):
    global icoord
    global iflag
    if playarea:
        screen_Image(0, 0, 1600, 1000)
    else:
        screen_Image(left, top, right, bottom)
    img_rgb = cv2.imread('images/screenshot.png')
    img_gray = cv2.cvtColor(img_rgb, cv2.COLOR_BGR2GRAY)
